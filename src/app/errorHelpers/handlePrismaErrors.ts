...
const getStatusCodeFromPrismaError = (errorCode: string): number => {
  ...
  //! Error => P2025, P2001, P2015, P2018: Not Found errors\
  if (["P2025", "P2025", "P2015", "P2018"].includes(errorCode)) {
    return status.NOT_FOUND;
  }

  //! Error => P1000, P6002: DB Authentication Error = 401 unauthorized
  if (["P1000", "P6002"].includes(errorCode)) {
    return status.UNAUTHORIZED;
  }

  //! Error => P1010, P6010: Access denied errors = 403 Forbidden
  if (["P1010", "P6010"].includes(errorCode)) {
    return status.FORBIDDEN;
  }

  //! Error => P6003: Prisma Accelerate Plan limit exceeded = 402 payment required
  if (errorCode === "P6003") {
    return status.PAYMENT_REQUIRED;
  }

  //! Error =>  P1008, P2004, P6004: Timeout error = 502 Gateway Timeout
  if (["P1008", "P2004", "P6004"].includes(errorCode)) {
    return status.GATEWAY_TIMEOUT;
  }

  //! Error => P5011 : Rate Limit Exceeded = 429 Too Many Requests
  if (errorCode === "P5011") {
    return status.TOO_MANY_REQUESTS;
  }

  //! Error => P6009 Response size limit exceeded = 413 Payload Too Large
  if (errorCode === "P6009") {
    return 413;
  }

  //! Error => P1xxx , P2024, P2037, P6008 : Connection errors
  if (errorCode.startsWith("P1") || ["P2024", "P2037", "P6008"].includes(errorCode)) {
    return status.SERVICE_UNAVAILABLE;
  }

  //! Error => P2XXX : except unhandled errors, Bad Request
  if (errorCode.startsWith("P2")) {
    return status.BAD_REQUEST;
  }

  //! Error => P3XXX, P4XXX : Internal Server Errors
