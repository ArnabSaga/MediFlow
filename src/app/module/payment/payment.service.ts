      ...
      if (session.payment_status === "paid" && result.invoiceUrl) {
        try {
          await sendEmail({
            ...
            templateData: {
              invoiceId:
                appointment.payment?.id?.substring(0, 8).toUpperCase() ||
                paymentId.substring(0, 8).toUpperCase(),
              date: new Date().toLocaleDateString(),
              status: "Paid",
              patient: {
                name: appointment.patient.name,
                id: appointment.patient.id?.substring(0, 8).toUpperCase() || "N/A",
                address: appointment.patient.address || "N/A",
                phone: appointment.patient.contactNumber || "N/A",
              },
              paymentDetails: {
                method: "Stripe",
                transactionId:
              ...
              items: [
                {
                  ...
                  description: `Appointment on ${new Date(appointment.schedule.startDateTime).toLocaleDateString()}`,
                  quantity: 1,
                  unitPrice: appointment.payment?.amount || 0,
                },
              ],
              summary: {
                subtotal: appointment.payment?.amount || 0,
                taxRate: 0,
                taxAmount: 0,
                discount: 0,
                total: appointment.payment?.amount || 0,
              },
